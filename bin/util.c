...
/*static void copy_nonblock(int src, int dst)
{
	char buf[PIPE_BUF];
	int r, w1, w;
	...
	while ((r = read(src, buf, sizeof(buf))) > 0) {
		w = 0;
		do {
			w1 = write(dst, buf + w, r - w);
			w += w1;
		} while (w1 > 0 && w < r);
	}
}*/

/* static int test_ioctl(int fd, unsigned int flags, int *ret, unsigned *features) */
/* { */
/* 	struct vecaps caps; */

/* 	caps.dst_flags = flags; */
/* 	*ret = ioctl(fd, CPT_TEST_VECAPS2, &caps); */
/* 	if (*ret >= 0) { */
/* 		*features = caps.features; */
/* 		return 0; */
/* 	} */
/* 	if (errno != ENOTTY) { */
/* 		log_sys(LOG_ERR, "ioctl(0x%x)", CPT_TEST_VECAPS2); */
/* 		return MIG_ERR_SYSTEM; */
/* 	} */
/* 	logger(LOG_DEBUG, "CPT_TEST_VECAPS2 is not supported"); */
/* 	*ret = ioctl(fd, CPT_TEST_VECAPS, flags); */
/* 	if (*ret >= 0) { */
/* 		*features = 0; */
/* 		return 0; */
/* 	} */
/* 	log_sys(LOG_ERR, "ioctl(0x%x)", CPT_TEST_VECAPS); */
/* 	return MIG_ERR_SYSTEM; */
/* } */
...
/* { */
/* 	int fd; */
/* 	int rc = MIG_ERR_SYSTEM; */
/* 	int error_pipe[2]; */

/* 	fd = open("/proc/cpt", O_RDWR); */
/* 	if (fd < 0) */
/* 		return putErr(MIG_ERR_SYSTEM, "kernel module vzcpt is not loaded."); */

/* 	init_pipes(error_pipe); */
/* 	if (pipe2(error_pipe, O_NONBLOCK) < 0) { */
/* 		log_sys(LOG_ERR, "pipe()"); */
/* 		goto finish; */
/* 	} */

/* 	if (ioctl(fd, CPT_SET_ERRORFD, error_pipe[1]) < 0) { */
/* 		log_sys(LOG_ERR, "ioctl(0x%x)", CPT_SET_ERRORFD); */
/* 		goto finish; */
/* 	} */
/* 	close_safe(&error_pipe[1]); */

/* 	if (ioctl(fd, CPT_SET_VEID, veid) < 0) { */
/* 		log_sys(LOG_ERR, "ioctl(0x%x)", CPT_SET_VEID); */
/* 		goto finish; */
/* 	} */

/* 	rc = test_ioctl(fd, flags, ret, features); */
/* 	copy_nonblock(error_pipe[0], STDERR_FILENO); */

/* finish: */
/* 	close(fd); */
/* 	close_pipes(error_pipe); */
