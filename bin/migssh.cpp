		...
		void **swapch)
{
	...
	if ((ret = vzsock_open_conn(ctx, (void *)sshArgs, swapch)))
		return putErr(MIG_ERR_VZSOCK,
			"vzsock_open_conn() return %d", ret);
...
int MigrateSshChannel::fwdStartSwapCli(const char *cmd, void **swapch)
{
	int rc = 0, ret;
	char buffer[BUFSIZ];
	...
	if (sock_connect(buffer, m_sFwdPort, VZMoptions.tmo.val, &sock)) {
		rc = putErr(MIG_ERR_CANT_CONNECT,
			"can't connect to host %s port %s", buffer, m_sFwdPort);
		goto cleanup_0;
	}

	/* read reply from channel */
	if ((rc = sock_read(sock, VZMoptions.tmo.val, '\n', buffer, &sz)))
		goto cleanup_1;
	...
	*swapch = malloc(sizeof(sock));
	memcpy(*swapch, &sock, sizeof(sock));
	logger(LOG_DEBUG, "Swap channel established");
	return 0;
cleanup_1:
	close(sock);
cleanup_0:
	/* on swap channel failure the migration may be continue,
	so will read all data fron incoming channel */
	ch_read_retcode(&ctx, conn);

	return rc;
}

void MigrateSshChannel::fwdCloseSwap(const void *swapch)
{
	if (swapch == NULL)
		return;
	close(*((int *)swapch));
...
int MigrateSshChannel::fwdStartSwapSrv(char * const argv[])
{
	int rc = 0;
	pid_t pid, chpid;
	int status;
