...
static int nbfd_write(int fd, long timeout, const char * data, size_t size)
{
	int rc;
	size_t sent;
	fd_set fds;
	...
	while (1) {
		...
		do {
			...
			if (rc == 0)
				return putErr(MIG_ERR_CONN_TIMEOUT,
					"%s : select() : timeout (%d sec)",
					__FUNCTION__, timeout);
			else if (rc <= 0)
				return putErr(MIG_ERR_CONN_BROKEN,
					"%s : select() : %m", __FUNCTION__);
