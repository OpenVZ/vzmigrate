...
#if 0
int MigrateChannel::createSslChannel(const char * args[])
{
	int rc = 0;
	int ret;
...
//	if (VZMoptions.transport == VZSOCK_SSL)
//		return 0;

	if ((addr.sin_addr.s_addr = inet_addr(VZMoptions.dst_addr))
		== INADDR_NONE)
	{
		/* need to resolve address */
		struct hostent *host;
		if ((host = gethostbyname(VZMoptions.dst_addr)) == NULL)
	...
	if ((ret = vzsock_set(&ctx, VZSOCK_DATA_ADDR,
